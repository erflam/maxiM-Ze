...
def process_library_match(Config, excel_path: Path) -> str:
    from pathlib import Path

    data_file = str(excel_path)
    output_file = os.path.splitext(data_file)[0] + "_with_library_matches.xlsx"

    mz_tol = Config.LIBRARY_MATCH_MZ_TOL
    rt_tol = Config.LIBRARY_MATCH_RT_TOL

    data_s1 = pd.read_excel(data_file, sheet_name=0)
    data_s2 = pd.read_excel(data_file, sheet_name=1)

    required_s1 = ["Group", "m/z", "Aligned_rt_apex", "Recluster"]
    missing_s1 = [c for c in required_s1 if c not in data_s1.columns]
    if missing_s1:
        raise ValueError(f"Summary Excel Sheet 1 missing columns: {missing_s1}. Found: {list(data_s1.columns)}")

    lib = read_library(Config.LIB_FILE)
    ...
    out = pd.concat([data_s1.copy(), matches], axis=1)
    ...
    fixed_cols = [
        "Processing Group", "MZ", "Adduct", "Aligned RT",
        "Library Match", "Formula", "Secondary Matches", "Recluster",
    ]
    ...
    print(f"[✔] Library match complete → {output_file}")
    print(f"    Tolerances used: MZ={mz_tol}, RT={rt_tol} min")
    return f"Library match export complete → {output_file}"
